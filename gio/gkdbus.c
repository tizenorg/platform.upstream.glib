...
/**
 * _g_kdbus_get_msg_buffer_ptr:
 *
 */
gchar *
_g_kdbus_get_msg_buffer_ptr (GKdbus  *kdbus)
{
  return (gchar *) kdbus->priv->kdbus_msg_items;
}
 ...
 */
gboolean
_g_kdbus_try_close (GKdbus  *kdbus,
                    GError **error)
{
  g_return_val_if_fail (G_IS_KDBUS (kdbus), FALSE);
...
g_kdbus_take_fd (GKdbus  *kdbus)
{
  ...
  /* connection's human-readable name (for debugging purposes)*/
  item = hello->items;
  ...
  /* TODO: validate bloom filters parameters */
  //if (hello->bloom.size != KDBUS_BLOOM_SIZE)
...
g_kdbus_alloc_memfd (GKdbus  *kdbus)
{
  ...
  size = ALIGN8(offsetof(struct kdbus_cmd_memfd_make, items)) +
               ...
               GError       **error)
{
  ...
  /*
   * check whether we should use memfd transport
   */
