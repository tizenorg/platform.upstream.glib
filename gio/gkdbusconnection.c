 ...
 */
gboolean
_g_kdbus_connection_is_connected (GKdbusConnection  *connection)
{
  return (!_g_kdbus_is_closed (connection->priv->kdbus));
}
...
g_kdbus_connection_dispose (GObject  *object)
{
  ...
  if (!connection->priv->closed)
    _g_kdbus_connection_close (connection, NULL, NULL);
...
g_kdbus_connection_real_close (GKdbusConnection *connection,
                               ...
                               GError          **error)
{
  return (_g_kdbus_try_close (connection->priv->kdbus, NULL));
}
 ...
 */
gboolean
_g_kdbus_connection_close (GKdbusConnection  *connection,
                           GCancellable      *cancellable,
                           GError           **error)
{
  ...
  if (class->close_fn)
    result = class->close_fn (connection, cancellable, error);
 ...
 */
void
_g_kdbus_connection_close_async (GIOStream            *stream,
                                 int                   io_priority,
                                 GCancellable         *cancellable,
                                 GAsyncReadyCallback   callback,
                                 gpointer              user_data)
  ...
  if (connection->priv->closed)
    {
      simple = g_simple_async_result_new (G_OBJECT (connection),
                                          callback,
                                          user_data,
                                          _g_kdbus_connection_close_async);
 ...
 */
gboolean
_g_kdbus_connection_close_finish (GIOStream     *stream,
                                  GAsyncResult  *result,
                                  GError       **error)
{
  GSimpleAsyncResult *simple;
  ...
  if (G_IS_SIMPLE_ASYNC_RESULT (result))
    {
      ...
      if (g_simple_async_result_get_source_tag (simple) == _g_kdbus_connection_close_async)
        return TRUE;
    }
